...
class _StudentDashboardState extends State<StudentDashboard> {
  ...
  Future<void> _loadCarryMark() async {
    final mark = await _dbService.getCarryMarkByStudentId(widget.user.studentId!);
    setState(() {
      _carryMark = mark;
      _isLoading = false;
    });
  ...
  double _calculateRequiredExamMark(double targetGrade) {
    ...
    final requiredExam = (targetGrade - (carryMarkTotal * 0.5)) / 0.5;
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (_carryMark == null)
                                    ...
                                    _buildGradesTable(isSmallScreen, isVerySmallScreen),
                                ],
                              ),
                            ),
                            SizedBox(height: isSmallScreen ? 16 : 24),
    ...
    if (isVerySmallScreen) {
      ...
      return Column(
        children: grades.map((g) {
          ...
          final targetGrade = (minVal + maxVal) / 2.0;
          ...
          Color getColor() {
            ...
            if (isEasy) return Colors.greenAccent;
            if (requiredExam < 70) return Colors.orangeAccent;
            return Colors.redAccent;
          }

          Color getBgColor() {
            if (isImpossible) return Colors.redAccent.withOpacity(0.1);
            if (requiredExam < 0) return Colors.greenAccent.withOpacity(0.1);
            if (isEasy) return Colors.greenAccent.withOpacity(0.1);
            if (requiredExam < 70) return Colors.orangeAccent.withOpacity(0.1);
